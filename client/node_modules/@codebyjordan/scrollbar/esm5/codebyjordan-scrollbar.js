...
import { InjectionToken, Inject, Injectable, Directive, ElementRef, Input, Optional, Renderer2, NgModule } from '@angular/core';
...
var ScrollbarService = (function () {
    ...
    ScrollbarService.prototype.initWheel = function (el) {
        var dommousescroll = Observable.fromEvent(el, 'DOMMouseScroll');
        var mousewheel = Observable.fromEvent(el, 'mousewheel');
        var wheel = Observable.fromEvent(el, 'wheel');
        return Observable.merge.apply(Observable, __spread([dommousescroll, mousewheel, wheel])).map(function (e) {
            e.preventDefault();
    ...
    ScrollbarService.prototype.initDrag = function (el, bar) {
        var _this = this;
        var observs;
        var mousemove = Observable.fromEvent(this.ws.window, 'mousemove');
        var mousedown = Observable.fromEvent(bar, 'mousedown');
        var mouseup = Observable.fromEvent(this.ws.window, 'mouseup');
        ...
        if (this.ws.isMobile) {
            var touchmove_1 = Observable.fromEvent(this.ws.window, 'touchmove');
            var touchstart = Observable.fromEvent(el, 'touchstart');
            var touchend_1 = Observable.fromEvent(this.ws.window, 'touchend');
            ...
            observs = {
                start: Observable.merge.apply(Observable, __spread([mousedrag, touchdrag])),
                end: Observable.merge.apply(Observable, __spread([mouseup, touchend_1])),
                move: Observable.merge.apply(Observable, __spread([mousemove, touchstart]))
            };
        }
        return observs;
    };
...
var CbjScrollbarDirective = (function () {
    function CbjScrollbarDirective(el, renderer, scroll, ws, router) {
        var _this = this;
        this.el = el;
...
export { DEFAULT_SCROLLBAR, ScrollbarConfig, WINDOW, _window, WindowService, ScrollbarService, CbjScrollbarDirective, CbjScrollbarModule, CbjScrollbarDirective as ɵa, ScrollbarService as ɵb, WINDOW as ɵd, _window as ɵe, WindowService as ɵc };
//# sourceMappingURL=codebyjordan-scrollbar.js.map
